...
namespace Car_Agency_Management.Pages
{
    public class AddCarModel : PageModel
    {
        ...
        public IActionResult OnPost()
        {
            ...
            var imageIndex = 0;
            while (Request.Form.ContainsKey($"Images[{imageIndex}]"))
            {
                var imageUrl = Request.Form[$"Images[{imageIndex}]"].ToString();
            ...
            // Generate carId if it's a new car
            if (string.IsNullOrEmpty(CarId))
            {
        ...
        {
            // Load car data from the same source as CarDetails
        ...
        {
            // Use the same data source as CarDetails
        ...
        private string GenerateCarId(string carName)
        {
            // Generate a URL-friendly ID from the car name
            return carName.ToLower()
                .Replace(" ", "-")
                .Replace("/", "-")
                .Replace("\\", "-")
                .Replace("&", "and")
